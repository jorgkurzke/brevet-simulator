...
st.title("🚴‍♂️ Brevet-Simulator mit GPX, FTP, ACP & Wind")
...
def apply_breaks(df, pause_df):
    ...
    for _, row in pause_df.iterrows():
        km_break = row["km"]
        dur_min = row["Dauer_min"]
        dur_s = dur_min * 60
        idx = df.index[df["cum_dist"] >= km_break * 1000]
        if len(idx) > 0:
            i = idx[0]
            df.loc[i:, "break_s"] += dur_s
...
def compute_controls(df, kontroll_df):
    ...
    if not controls:
        return pd.DataFrame(columns=["km", "Name", "sim_time_s", "acp_limit_s"])
...
def compute_stats(points):
    ...
    df["time_s"] = df["dist"] / df["speed_mps"]
    df["time_s"].fillna(0, inplace=True)
